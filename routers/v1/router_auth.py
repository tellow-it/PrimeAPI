...
@router_auth.post("/login-telegram", status_code=200, response_model=TokenSchema)
async def login(login_data: LoginSchema):
    user_obj = await User.get_or_none(telephone=login_data.telephone)
    if user_obj:
        if verify_password(login_data.password, user_obj.password):
...
async def get_new_access_token(refresh_token: HTTPAuthorizationCredentials):
    ...
    exp_minutes = 60 * 2
    return {"access_token": create_access_token(token_data, exp_minutes=exp_minutes),
            "token_type": "Bearer"}
